...
public class OrcCharacteristic extends Characteristic {
    ...
    public OrcCharacteristic(PlayerCustomization playerCustomization, String name, int maxVariant) {
        super(playerCustomization, name, maxVariant);
        if(FMLLoader.getDist() == Dist.CLIENT) {
            this.maleModel = new MaleOrcModel(Minecraft.getInstance().getEntityModels().bakeLayer(ModModelLayers.MALE_ORC));
            this.femaleModel = new FemaleOrcModel(Minecraft.getInstance().getEntityModels().bakeLayer(ModModelLayers.FEMALE_ORC));
        }
    }
    ...
    public void translateToArm(PoseStack poseStack, HumanoidArm arm) {
        ...
        poseStack.translate(flag? -0.1 : 0.1,0.125F,-0.9);
        poseStack.mulPose(Axis.YP.rotationDegrees(-180.0F));
        poseStack.mulPose(Axis.XP.rotationDegrees(90.0F));
    }
    ...
    public ResourceLocation getTexture(Player player){
        ResourceLocation baseTexture = ResourceLocation.fromNamespaceAndPath(FantasyBeyond.MOD_ID, "textures/entity/customization/race/orc/" + name + "/" + (getPlayerCustomization().isMale() ? "male_" : "female_") + getVariant() + ".png");
