...
public class FemaleHumanModel<T extends Entity> extends CustomizationModel<T> {
	...
	public void copyFrom(CustomizationModel model) {
		root.copyFrom(((FemaleHumanModel) model).root);
		body.copyFrom(((FemaleHumanModel) model).body);
		right_leg.copyFrom(((FemaleHumanModel) model).right_leg);
		left_leg.copyFrom(((FemaleHumanModel) model).left_leg);
		head.copyFrom(((FemaleHumanModel) model).head);
		left_arm.copyFrom(((FemaleHumanModel) model).left_arm);
		right_arm.copyFrom(((FemaleHumanModel) model).right_arm);
	}
	...
	public static LayerDefinition createBodyLayer(CubeDeformation cubeDeformation) {
		...
		PartDefinition left_leg = root.addOrReplaceChild("left_leg", CubeListBuilder.create().texOffs(16, 52).addBox(-2.0F, 0.0F, -2.0F, 4.0F, 12.0F, 4.0F, cubeDeformation)
